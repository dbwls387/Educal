  ...
  setup (props) {
    ...
    const getTestData = async () => {
      await axios({
        ...
        .then((res) => {
          console.log(res.data.retInfo, '차트 데이터')
    ...
    const labels = recentTestData.value.slice(0, 5).map(data => data.testName)
    const scores = recentTestData.value.slice(0, 5).map(data => data.score);
    const avgscores = recentTestData.value.slice(0, 5).map(data => data.avgScore)
    console.log(scores)
    new Chart(ctx2, {
      type: 'line',
      data: {
